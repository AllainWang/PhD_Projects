%%%%%%
%%%This script is created to generate the error rate of BTF model.
%%%Data base is generated by setting t-3, t-6 and t-8 are important.
%%%There are 100 data sets in the data base.
%%%First 1000 training data, the rest 300 testing data.
%%%By Zhongzhen Wang in May 2018.
...
class_error = zeros(1, 100);

ddata = importdata('cat_gen_368.txt');
for iii = 1:100
y = ddata.data(:,iii);
...
Nnew=N-qmax;
X0new=zeros(Nnew,pnew);            
Y0new=Y0((qmax+1):N);           
for j=1:qmax                          
    X0new(:,j)=Y0(qmax+1-j:(N-j));     
end                                    
for j=(qmax+1):pnew                   
